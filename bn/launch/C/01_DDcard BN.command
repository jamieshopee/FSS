...
readonly FSS_C01_URL="${FSS_VIEWER_URL}?type=C&bn=01_DDcard%20BN"
...
open_c01_viewer() {
  if ! "${FSS_OPEN}" "${FSS_C01_URL}"; then
    echo "FSS BN Viewer 已啟動，但無法自動開啟瀏覽器。"
    echo "請開啟：${FSS_C01_URL}"
    return 1
  fi
}
...
if viewer_is_ready; then
  open_c01_viewer || pause_before_exit
  exit
fi
...
if ! open_c01_viewer; then
  pause_before_exit
  exit 1
fi

echo
echo "FSS BN Viewer 已啟動：${FSS_C01_URL}"
