...
readonly FSS_C03_URL="${FSS_VIEWER_URL}?type=C&bn=03_Coin%20page%20BN"
...
open_c03_viewer() {
  if ! "${FSS_OPEN}" "${FSS_C03_URL}"; then
    echo "FSS BN Viewer 已啟動，但無法自動開啟瀏覽器。"
    echo "請開啟：${FSS_C03_URL}"
    return 1
  fi
}
...
if viewer_is_ready; then
  open_c03_viewer || pause_before_exit
  exit
fi
...
if ! open_c03_viewer; then
  pause_before_exit
  exit 1
fi

echo
echo "FSS BN Viewer 已啟動：${FSS_C03_URL}"
