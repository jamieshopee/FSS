...
readonly FSS_C13_URL="${FSS_VIEWER_URL}?type=C&bn=13_Skinny%20BN_APP"
...
open_c13_viewer() {
  if ! "${FSS_OPEN}" "${FSS_C13_URL}"; then
    echo "FSS BN Viewer 已啟動，但無法自動開啟瀏覽器。"
    echo "請開啟：${FSS_C13_URL}"
    return 1
  fi
}
...
if viewer_is_ready; then
  open_c13_viewer || pause_before_exit
  exit
fi

if "${FSS_CURL}" --silent --max-time 1 "${FSS_BASE_URL}/" >/dev/null 2>&1; then
  echo "Port 4173 已被其他程式佔用，無法啟動 FSS BN Viewer。"
  pause_before_exit
  exit 1
fi
...
if ! open_c13_viewer; then
  pause_before_exit
  exit 1
fi

echo
echo "FSS BN Viewer 已啟動：${FSS_C13_URL}"
