...
readonly FSS_D06_URL="${FSS_VIEWER_URL}?type=D&bn=06_IG"
...
open_d06_viewer() {
  if ! "${FSS_OPEN}" "${FSS_D06_URL}"; then
    echo "FSS BN Viewer 已啟動，但無法自動開啟瀏覽器。"
    echo "請開啟：${FSS_D06_URL}"
    return 1
  fi
}
...
if viewer_is_ready; then
  open_d06_viewer || pause_before_exit
  exit
fi
...
if ! open_d06_viewer; then
  pause_before_exit
  exit 1
fi

echo
echo "FSS BN Viewer 已啟動：${FSS_D06_URL}"
