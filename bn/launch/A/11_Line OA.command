...
readonly FSS_A11_URL="${FSS_VIEWER_URL}?type=A&bn=11_Line%20OA"
...
open_a11_viewer() {
  if ! "${FSS_OPEN}" "${FSS_A11_URL}"; then
    echo "FSS BN Viewer 已啟動，但無法自動開啟瀏覽器。"
    echo "請開啟：${FSS_A11_URL}"
    return 1
  fi
}
...
if viewer_is_ready; then
  open_a11_viewer || pause_before_exit
  exit
fi
...
if ! open_a11_viewer; then
  pause_before_exit
  exit 1
fi

echo
echo "FSS BN Viewer 已啟動：${FSS_A11_URL}"
