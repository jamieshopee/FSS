...
readonly FSS_A17_URL="${FSS_VIEWER_URL}?type=A&bn=17_%E9%96%80%E6%AA%BB%E8%A1%A8"
...
open_a17_viewer() {
  if ! "${FSS_OPEN}" "${FSS_A17_URL}"; then
    echo "FSS BN Viewer 已啟動，但無法自動開啟瀏覽器。"
    echo "請開啟：${FSS_A17_URL}"
    return 1
  fi
}
...
if viewer_is_ready; then
  open_a17_viewer || pause_before_exit
  exit
fi
...
if ! open_a17_viewer; then
  pause_before_exit
  exit 1
fi

echo
echo "FSS BN Viewer 已啟動：${FSS_A17_URL}"
