...
readonly FSS_A02_URL="${FSS_VIEWER_URL}?type=A&bn=02_MALL%20HBN"
...
open_a02_viewer() {
  if ! "${FSS_OPEN}" "${FSS_A02_URL}"; then
    echo "FSS BN Viewer 已啟動，但無法自動開啟瀏覽器。"
    echo "請開啟：${FSS_A02_URL}"
    return 1
  fi
}
...
if viewer_is_ready; then
  open_a02_viewer || pause_before_exit
  exit
fi
...
if ! open_a02_viewer; then
  pause_before_exit
  exit 1
fi

echo
echo "FSS BN Viewer 已啟動：${FSS_A02_URL}"
