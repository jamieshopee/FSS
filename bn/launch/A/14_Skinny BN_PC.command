...
readonly FSS_A14_URL="${FSS_VIEWER_URL}?type=A&bn=14_Skinny%20BN_PC"
...
open_a14_viewer() {
  if ! "${FSS_OPEN}" "${FSS_A14_URL}"; then
    echo "FSS BN Viewer 已啟動，但無法自動開啟瀏覽器。"
    echo "請開啟：${FSS_A14_URL}"
    return 1
  fi
}
...
if viewer_is_ready; then
  open_a14_viewer || pause_before_exit
  exit
fi
...
if ! open_a14_viewer; then
  pause_before_exit
  exit 1
fi

echo
echo "FSS BN Viewer 已啟動：${FSS_A14_URL}"
