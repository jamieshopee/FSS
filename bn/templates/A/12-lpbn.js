export const LPBN_WIDTH = 1200;
export const LPBN_HEIGHT = 550;
...
export const LPBN_BACKGROUND = Object.freeze({
...
export const LPBN_LAYOUT = Object.freeze({
...
function assertSpecificationFitsCanvas() {
  if (LPBN_WIDTH !== 1200 || LPBN_HEIGHT !== 550) {
    throw new Error("A－12 正式 Canvas 尺寸無效，已停止 Template render。");
  }
  ...
  if (
    ...
    throw new Error("A－12 正式底圖 placement 無效，已停止 Template render。");
  }
  ...
  for (const [name, box] of Object.entries(LPBN_LAYOUT)) {
    ...
    if (!values.every(Number.isFinite) || box.width <= 0 || box.height <= 0) {
      throw new Error(`A－12 ${name} 正式文字框無效，已停止 Template render。`);
    }
    if (
      box.left < 0 ||
      box.top < 0 ||
      ...
      throw new Error(`A－12 ${name} 正式文字框超出 Canvas，已停止 Template render。`);
    }
  }
}
...
function measureRun(context, text, font, symbol = false) {
  ...
  if (!values.every(Number.isFinite)) {
    throw new Error("A－12 正式字型 metrics 無效，已停止 Template render。");
  }
...
function boundaryGlyphInkBottom(context, run, fromStart) {
  ...
  for (const glyph of glyphs) {
    ...
    if (!values.every(Number.isFinite)) {
      throw new Error("A－12 副標 glyph metrics 無效，已停止 Template render。");
    }
...
function validateLeftCenteredInkFitsBox(label, box, inkWidth, inkHeight) {
  if (
    ...
    throw new Error(`A－12 ${label} 正式文字 ink metrics 無效，已停止 Template render。`);
  }
...
function drawLpbnMediumText(context, headline, protectionText) {
  ...
  if (mediumCanvas.width !== 2400 || mediumCanvas.height !== 1100) {
    throw new Error("A－12 Medium 暫存 Canvas 尺寸無效，已停止 Template render。");
  }

  const mediumContext = mediumCanvas.getContext("2d");
  if (!mediumContext) {
    throw new Error("無法建立 A－12 Medium 暫存 Canvas 2D context。");
  }
...
function assertFontsReady() {
  if (
    ...
    throw new Error("正式字型尚未載入，已停止 A－12 Template render。");
  }
}

export async function waitForLpbnFonts() {
  if (!hasFontFaceSetCapabilities()) {
    throw new Error("瀏覽器不支援正式字型載入檢查，已停止 A－12 Template render。");
  }
...
export function renderLpbn(
  ...
  if (!(canvas instanceof HTMLCanvasElement)) {
    throw new TypeError("A－12 Template 需要 HTMLCanvasElement。");
  }
  if (!(backgroundImage instanceof HTMLImageElement)) {
    throw new TypeError("A－12 Template 需要已載入的 HTMLImageElement 底圖。");
  }
  if (!backgroundImage.complete || backgroundImage.naturalWidth === 0) {
    throw new Error("A－12 正式底圖尚未完成解碼。");
  }
  if (
    backgroundImage.naturalWidth !== BACKGROUND_WIDTH ||
    backgroundImage.naturalHeight !== BACKGROUND_HEIGHT
  ) {
    throw new Error("A－12 正式底圖必須為 1200 × 550px。");
  }
  ...
  if (canvas.width !== 1200 || canvas.height !== 550) {
    throw new Error("A－12 正式 Canvas 尺寸設定失敗，已停止 Template render。");
  }

  const context = canvas.getContext("2d");
  if (!context) throw new Error("無法建立 A－12 Canvas 2D context。");
