export const MALL_HBN_WIDTH = 1200;
export const MALL_HBN_HEIGHT = 360;
...
export const MALL_HBN_LAYOUT = Object.freeze({
...
function drawMallHbnMediumText(context, headline, protectionText) {
  ...
  if (!mediumContext) {
    throw new Error("無法建立 A－02 Medium 暫存 Canvas 2D context。");
  }
...
function assertFontsReady() {
  if (
    ...
    throw new Error("正式字型尚未載入，已停止 A－02 Template render。");
  }
}

export async function waitForMallHbnFonts() {
  if (!document.fonts) {
    throw new Error("瀏覽器不支援正式字型載入檢查，已停止 A－02 Template render。");
  }
...
export function renderMallHbn(
  ...
  if (!(canvas instanceof HTMLCanvasElement)) {
    throw new TypeError("A－02 Template 需要 HTMLCanvasElement。");
  }
  if (!(backgroundImage instanceof HTMLImageElement)) {
    throw new TypeError("A－02 Template 需要已載入的 HTMLImageElement 底圖。");
  }
  if (
    backgroundImage.naturalWidth !== MALL_HBN_WIDTH ||
    backgroundImage.naturalHeight !== MALL_HBN_HEIGHT
  ) {
    throw new Error("A－02 正式底圖必須為 1200 × 360px。");
  }
  ...
  if (!context) throw new Error("無法建立 A－02 Canvas 2D context。");
