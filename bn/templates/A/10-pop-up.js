export const POP_UP_WIDTH = 580;
export const POP_UP_HEIGHT = 720;
...
export const POP_UP_BACKGROUND = Object.freeze({
...
export const POP_UP_LAYOUT = Object.freeze({
...
function assertSpecificationFitsCanvas() {
  if (POP_UP_WIDTH !== 580 || POP_UP_HEIGHT !== 720) {
    throw new Error("A－10 正式 Canvas 尺寸無效，已停止 Template render。");
  }
  ...
  if (
    ...
    throw new Error("A－10 正式底圖 placement 無效，已停止 Template render。");
  }
  ...
  for (const [name, box] of Object.entries(POP_UP_LAYOUT)) {
    ...
    if (!values.every(Number.isFinite) || box.width <= 0 || box.height <= 0) {
      throw new Error(`A－10 ${name} 正式文字框無效，已停止 Template render。`);
    }
    if (
      box.left < 0 ||
      box.top < 0 ||
      ...
      throw new Error(`A－10 ${name} 正式文字框超出 Canvas，已停止 Template render。`);
    }
  }
}
...
function measureRun(context, text, font, symbol = false) {
  ...
  if (!values.every(Number.isFinite)) {
    throw new Error("A－10 正式字型 metrics 無效，已停止 Template render。");
  }
...
function boundaryGlyphInkBottom(context, run, fromStart) {
  ...
  for (const glyph of glyphs) {
    ...
    if (!Number.isFinite(metrics.actualBoundingBoxDescent)) {
      throw new Error("A－10 副標 glyph metrics 無效，已停止 Template render。");
    }
...
function validateCenteredInkFitsBox(box, inkWidth, inkHeight) {
  ...
  if (!values.every(Number.isFinite) || inkWidth < 0 || inkHeight < 0) {
    throw new Error("A－10 正式文字 ink metrics 無效，已停止 Template render。");
  }
...
function drawPopUpMediumText(context, headline, protectionText) {
  ...
  if (mediumCanvas.width !== 1160 || mediumCanvas.height !== 1440) {
    throw new Error("A－10 Medium 暫存 Canvas 尺寸無效，已停止 Template render。");
  }

  const mediumContext = mediumCanvas.getContext("2d");
  if (!mediumContext) {
    throw new Error("無法建立 A－10 Medium 暫存 Canvas 2D context。");
  }
...
function assertFontsReady() {
  if (
    ...
    throw new Error("正式字型尚未載入，已停止 A－10 Template render。");
  }
}

export async function waitForPopUpFonts() {
  if (!document.fonts) {
    throw new Error("瀏覽器不支援正式字型載入檢查，已停止 A－10 Template render。");
  }
...
export function renderPopUp(
  ...
  if (!(canvas instanceof HTMLCanvasElement)) {
    throw new TypeError("A－10 Template 需要 HTMLCanvasElement。");
  }
  if (!(backgroundImage instanceof HTMLImageElement)) {
    throw new TypeError("A－10 Template 需要已載入的 HTMLImageElement 底圖。");
  }
  if (!backgroundImage.complete || backgroundImage.naturalWidth === 0) {
    throw new Error("A－10 正式底圖尚未完成解碼。");
  }
  if (
    backgroundImage.naturalWidth !== BACKGROUND_WIDTH ||
    backgroundImage.naturalHeight !== BACKGROUND_HEIGHT
  ) {
    throw new Error("A－10 正式底圖必須為 475 × 673px。");
  }
  ...
  if (canvas.width !== POP_UP_WIDTH || canvas.height !== POP_UP_HEIGHT) {
    throw new Error("A－10 正式 Canvas 尺寸設定失敗，已停止 Template render。");
  }

  const context = canvas.getContext("2d");
  if (!context) throw new Error("無法建立 A－10 Canvas 2D context。");
