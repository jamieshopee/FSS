export const SPX_TVBN_2_WIDTH = 1599;
export const SPX_TVBN_2_HEIGHT = 1080;
...
export const SPX_TVBN_2_LAYOUT = Object.freeze({
...
function assertLayoutFitsCanvas() {
  for (const [name, box] of Object.entries(SPX_TVBN_2_LAYOUT)) {
    ...
    if (!values.every(Number.isFinite) || box.width <= 0 || box.height <= 0) {
      throw new Error(`A－09 ${name} 正式文字框無效，已停止 Template render。`);
    }
    if (
      box.left < 0 ||
      box.top < 0 ||
      ...
      throw new Error(`A－09 ${name} 正式文字框超出 Canvas，已停止 Template render。`);
    }
  }
}
...
function measureRun(context, text, font, symbol = false) {
  ...
  if (!values.every(Number.isFinite)) {
    throw new Error("A－09 正式字型 metrics 無效，已停止 Template render。");
  }
...
function boundaryGlyphInkBottom(context, run, fromStart) {
  ...
  for (const glyph of glyphs) {
    ...
    if (!Number.isFinite(metrics.actualBoundingBoxDescent)) {
      throw new Error("A－09 副標 glyph metrics 無效，已停止 Template render。");
    }
...
function drawSpxTvbn2MediumText(context, headline, protectionText) {
  ...
  if (!mediumContext) {
    throw new Error("無法建立 A－09 Medium 暫存 Canvas 2D context。");
  }
...
function assertFontsReady() {
  if (
    ...
    throw new Error("正式字型尚未載入，已停止 A－09 Template render。");
  }
}

export async function waitForSpxTvbn2Fonts() {
  if (!document.fonts) {
    throw new Error("瀏覽器不支援正式字型載入檢查，已停止 A－09 Template render。");
  }
...
export function renderSpxTvbn2(
  ...
  if (!(canvas instanceof HTMLCanvasElement)) {
    throw new TypeError("A－09 Template 需要 HTMLCanvasElement。");
  }
  if (!(backgroundImage instanceof HTMLImageElement)) {
    throw new TypeError("A－09 Template 需要已載入的 HTMLImageElement 底圖。");
  }
  if (!backgroundImage.complete || backgroundImage.naturalWidth === 0) {
    throw new Error("A－09 正式底圖尚未完成解碼。");
  }
  if (
    backgroundImage.naturalWidth !== BACKGROUND_WIDTH ||
    backgroundImage.naturalHeight !== BACKGROUND_HEIGHT
  ) {
    throw new Error("A－09 正式底圖必須為 1599 × 1080px。");
  }
  ...
  if (!context) throw new Error("無法建立 A－09 Canvas 2D context。");
