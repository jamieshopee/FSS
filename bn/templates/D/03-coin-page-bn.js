// 樣式 D－03（03_Coin page BN）正式 Template definition。
// 文字 renderer 行為基準為已驗證的 bn/templates/A/03-coin-page-bn.js；固定 Logo 處理基準為
// bn/templates/D/02-mall-hbn.js 的靠左 contain 模式。本檔不修改、不 import 上述任何檔案。
// 與 A 版同版位的差異只有一處：新增固定 Logo（三文字 geometry／樣式／對齊完全相同）。
// 固定素材（底圖、Logo）一律由 caller 載入完成後以 images object 傳入，本檔不自行載圖、不做 cache。
const COIN_PAGE_BN_WIDTH = 1200;
const COIN_PAGE_BN_HEIGHT = 391;
...
// D－03 placement。三個文字 box 與 A 版同版位完全相同（left／top／width／height 一字未改）；
// logo box 為 D－03 新增，不含 font／color，因為它不是文字。
const COIN_PAGE_BN_LAYOUT = Object.freeze({
  logo: Object.freeze({
    left: 92,
    top: 107,
    width: 351,
...
// D－03 固定 Logo：以 contain 等比例縮放完整落入 logo box，再**靠左靠上**貼齊。
// scale 取兩軸比例的最小值；現行素材 784 × 112 對 351 × 50 box 時
// scale = min(351/784, 50/112) = 25/56，destination 恰為 350 × 50、x = 92、y = 107。
// 水平明確靠左：destinationX = box.left；不套用水平置中公式（置中會使 x 落在半像素上）。
...
function drawCoinPageBnLogo(context, logoImage, box) {
...
function drawCoinPageBnMediumText(context, headline, protectionText) {
  ...
  if (!mediumContext) {
    throw new Error("無法建立 D－03 Medium 暫存 Canvas 2D context。");
  }
...
function assertFontsReady() {
  if (
    ...
    throw new Error("正式字型尚未載入，已停止 D－03 Template render。");
  }
}

export async function waitForCoinPageBnFonts() {
  if (!document.fonts) {
    throw new Error("瀏覽器不支援正式字型載入檢查，已停止 D－03 Template render。");
  }
...
export function renderCoinPageBn(
  ...
  // 固定素材以 images object 傳入（沿用 A－17／D－01／D－02 既有 images-object invocation 慣例）。
  const { backgroundImage, logoImage } =
    images && typeof images === "object" ? images : {};

  if (!(backgroundImage instanceof HTMLImageElement)) {
    throw new TypeError("D－03 Template 需要已載入的 HTMLImageElement 底圖。");
  }
  if (!backgroundImage.complete || backgroundImage.naturalWidth === 0) {
    throw new Error("D－03 正式底圖尚未完成解碼。");
  }
  if (
    backgroundImage.naturalWidth !== COIN_PAGE_BN_WIDTH ||
    backgroundImage.naturalHeight !== COIN_PAGE_BN_HEIGHT
  ) {
    throw new Error("D－03 正式底圖必須為 1200 × 391px。");
  }

  // 固定 Logo 為 D－03 的必要組成，不是 optional，缺失或未解碼一律明確失敗。
  // 這裡只驗證「有效且已載入的圖片」；不得驗證原始尺寸等於特定值或等於 destination box。
  // 現行素材為 784 × 112，但那只是 asset 實證，不是 Logo loader 的限制。
  if (!(logoImage instanceof HTMLImageElement)) {
    throw new TypeError("D－03 Template 需要已載入的 HTMLImageElement 固定 Logo。");
  }
  if (
    ...
    throw new Error("D－03 正式 Logo 尚未完成解碼。");
  }
  ...
  if (
    ...
    throw new Error("D－03 正式 Canvas 尺寸設定失敗，已停止 Template render。");
  }
  const context = canvas.getContext("2d");
  if (!context) throw new Error("無法建立 D－03 Canvas 2D context。");
  ...
  drawCoinPageBnLogo(context, logoImage, COIN_PAGE_BN_LAYOUT.logo);
