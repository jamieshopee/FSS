...
import {
  MSBN_HEIGHT,
  MSBN_WIDTH,
  renderMsbn as renderAMsbn,
  waitForMsbnFonts as waitForAMsbnFonts
} from "../A/05-msbn.js";

const COUNTDOWN_FONT = '60pt "ShopeeNotoSans Bold"';
...
const COUNTDOWN_TARGET_X = 864;
const COUNTDOWN_TARGET_Y = 278;
const COUNTDOWN_VISUAL_SCALE = 0.94;
...
function assertCountdownFontReady() {
  if (
    ...
    throw new Error("正式倒數字型尚未載入，已停止 C－05 Template render。");
  }
}
...
function drawCountdown(context, text) {
  ...
  if (!metricValues.every(Number.isFinite)) {
    throw new Error("瀏覽器無法取得 C－05 倒數文字的正式 glyph metrics。");
  }
  ...
  if (inkRight <= inkLeft || inkBottom <= inkTop) {
    throw new Error("C－05 倒數文字沒有可用的正式 glyph ink bounds。");
  }
...
export async function waitForMsbnFonts() {
  await waitForAMsbnFonts();
  if (!document.fonts) {
    throw new Error("瀏覽器不支援正式字型載入檢查，已停止 C－05 Template render。");
  }
...
export function renderMsbn(
  ...
  if (!isValidCCountdown(cCountdownText)) {
    throw new TypeError("C－05 倒數天數只允許完整字串 0天～9天。");
  }

  assertCountdownFontReady();
  renderAMsbn(canvas, backgroundImage, {
    headline,
    subheadline,
    protectionText
  });

  if (canvas.width !== MSBN_WIDTH || canvas.height !== MSBN_HEIGHT) {
    throw new Error("C－05 正式 Canvas 必須為 1200 × 400px。");
  }
  const context = canvas.getContext("2d");
  if (!context) throw new Error("無法建立 C－05 Canvas 2D context。");

  drawCountdown(context, cCountdownText);
}

export { MSBN_HEIGHT, MSBN_WIDTH };
