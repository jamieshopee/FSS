...
import {
  LINE_OA_HEIGHT,
  LINE_OA_WIDTH,
  renderLineOa as renderALineOa,
  waitForLineOaFonts as waitForALineOaFonts
} from "../A/11-line-oa.js";

const COUNTDOWN_FONT = '87pt "ShopeeNotoSans Bold"';
...
const COUNTDOWN_TARGET_X = 551;
const COUNTDOWN_TARGET_Y = 775.5;
...
function assertCountdownFontReady() {
  if (
    ...
    throw new Error("正式倒數字型尚未載入，已停止 C－11 Template render。");
  }
}
...
function drawCountdown(context, text) {
  ...
  if (!metricValues.every(Number.isFinite)) {
    throw new Error("瀏覽器無法取得 C－11 倒數文字的正式 glyph metrics。");
  }
  ...
  if (inkRight <= inkLeft || inkBottom <= inkTop) {
    throw new Error("C－11 倒數文字沒有可用的正式 glyph ink bounds。");
  }
...
export async function waitForLineOaFonts() {
  await waitForALineOaFonts();
  if (!document.fonts) {
    throw new Error("瀏覽器不支援正式字型載入檢查，已停止 C－11 Template render。");
  }
...
export function renderLineOa(
  ...
  if (!isValidCCountdown(cCountdownText)) {
    throw new TypeError("C－11 倒數天數只允許完整字串 0天～9天。");
  }
  ...
  if (canvas.width !== LINE_OA_WIDTH || canvas.height !== LINE_OA_HEIGHT) {
    throw new Error("C－11 正式 Canvas 必須為 1040 × 1040px。");
  }
  const context = canvas.getContext("2d");
  if (!context) throw new Error("無法建立 C－11 Canvas 2D context。");

  drawCountdown(context, cCountdownText);
}

export { LINE_OA_HEIGHT, LINE_OA_WIDTH };
