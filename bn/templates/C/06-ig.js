...
import {
  IG_HEIGHT,
  IG_WIDTH,
  renderIg as renderAIg,
  waitForIgFonts as waitForAIgFonts
} from "../A/06-ig.js";

const COUNTDOWN_FONT = '112pt "ShopeeNotoSans Bold"';
...
const COUNTDOWN_TARGET_X = 463;
const COUNTDOWN_TARGET_Y = 1168;
...
function assertCountdownFontReady() {
  if (
    ...
    throw new Error("正式倒數字型尚未載入，已停止 C－06 Template render。");
  }
}
...
function drawCountdown(context, text) {
  ...
  if (!metricValues.every(Number.isFinite)) {
    throw new Error("瀏覽器無法取得 C－06 倒數文字的正式 glyph metrics。");
  }
  ...
  if (inkRight <= inkLeft || inkBottom <= inkTop) {
    throw new Error("C－06 倒數文字沒有可用的正式 glyph ink bounds。");
  }
...
export async function waitForIgFonts() {
  await waitForAIgFonts();
  if (!document.fonts) {
    throw new Error("瀏覽器不支援正式字型載入檢查，已停止 C－06 Template render。");
  }
...
export function renderIg(
  ...
  if (!isValidCCountdown(cCountdownText)) {
    throw new TypeError("C－06 倒數天數只允許完整字串 0天～9天。");
  }

  assertCountdownFontReady();
  renderAIg(canvas, backgroundImage, {
    headline,
    subheadline,
    protectionText
  });
  ...
  if (canvas.width !== IG_WIDTH || canvas.height !== IG_HEIGHT) {
    throw new Error("C－06 正式 Canvas 必須為 900 × 1600px。");
  }
  const context = canvas.getContext("2d");
  if (!context) throw new Error("無法建立 C－06 Canvas 2D context。");

  drawCountdown(context, cCountdownText);
}

export { IG_HEIGHT, IG_WIDTH };
