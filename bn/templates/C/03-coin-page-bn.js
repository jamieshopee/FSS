...
import {
  COIN_PAGE_BN_HEIGHT,
  COIN_PAGE_BN_WIDTH,
  renderCoinPageBn as renderACoinPageBn,
  waitForCoinPageBnFonts as waitForACoinPageBnFonts
} from "../A/03-coin-page-bn.js";
...
const COUNTDOWN_TARGET_X = 863.5;
const COUNTDOWN_TARGET_Y = 276.5;
...
function assertCountdownFontReady() {
  if (
    ...
    throw new Error("正式倒數字型尚未載入，已停止 C－03 Template render。");
  }
}
...
function drawCountdown(context, text) {
  ...
  if (!metricValues.every(Number.isFinite)) {
    throw new Error("瀏覽器無法取得 C－03 倒數文字的正式 glyph metrics。");
  }
  ...
  if (inkRight <= inkLeft || inkBottom <= inkTop) {
    throw new Error("C－03 倒數文字沒有可用的正式 glyph ink bounds。");
  }
...
export async function waitForCoinPageBnFonts() {
  await waitForACoinPageBnFonts();
  if (!document.fonts) {
    throw new Error("瀏覽器不支援正式字型載入檢查，已停止 C－03 Template render。");
  }
...
export function renderCoinPageBn(
  ...
  if (!isValidCCountdown(cCountdownText)) {
    throw new TypeError("C－03 倒數天數只允許完整字串 0天～9天。");
  }
  ...
  if (
    ...
    throw new Error("C－03 正式 Canvas 必須為 1200 × 391px。");
  }
  const context = canvas.getContext("2d");
  if (!context) throw new Error("無法建立 C－03 Canvas 2D context。");

  drawCountdown(context, cCountdownText);
}

export { COIN_PAGE_BN_HEIGHT, COIN_PAGE_BN_WIDTH };
