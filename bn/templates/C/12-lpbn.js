...
import {
  LPBN_HEIGHT,
  LPBN_WIDTH,
  renderLpbn as renderALpbn,
  waitForLpbnFonts as waitForALpbnFonts
} from "../A/12-lpbn.js";

const COUNTDOWN_FONT = '74pt "ShopeeNotoSans Bold"';
...
const COUNTDOWN_TARGET_X = 876;
const COUNTDOWN_TARGET_Y = 365.5;
...
function assertCountdownFontReady() {
  if (
    ...
    throw new Error("正式倒數字型尚未載入，已停止 C－12 Template render。");
  }
}
...
function drawCountdown(context, text) {
  ...
  if (!metricValues.every(Number.isFinite)) {
    throw new Error("瀏覽器無法取得 C－12 倒數文字的正式 glyph metrics。");
  }
  ...
  if (inkRight <= inkLeft || inkBottom <= inkTop) {
    throw new Error("C－12 倒數文字沒有可用的正式 glyph ink bounds。");
  }
...
export async function waitForLpbnFonts() {
  await waitForALpbnFonts();
  if (!document.fonts) {
    throw new Error("瀏覽器不支援正式字型載入檢查，已停止 C－12 Template render。");
  }
...
export function renderLpbn(
  ...
  if (!isValidCCountdown(cCountdownText)) {
    throw new TypeError("C－12 倒數天數只允許完整字串 0天～9天。");
  }

  assertCountdownFontReady();
  renderALpbn(canvas, backgroundImage, {
    headline,
    subheadline,
    protectionText
  });

  if (canvas.width !== LPBN_WIDTH || canvas.height !== LPBN_HEIGHT) {
    throw new Error("C－12 正式 Canvas 必須為 1200 × 550px。");
  }
  const context = canvas.getContext("2d");
  if (!context) throw new Error("無法建立 C－12 Canvas 2D context。");

  drawCountdown(context, cCountdownText);
}

export { LPBN_HEIGHT, LPBN_WIDTH };
