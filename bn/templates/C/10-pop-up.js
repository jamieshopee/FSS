...
import {
  POP_UP_HEIGHT,
  POP_UP_WIDTH,
  renderPopUp as renderAPopUp,
  waitForPopUpFonts as waitForAPopUpFonts
} from "../A/10-pop-up.js";
...
const COUNTDOWN_TARGET_X = 298.5;
const COUNTDOWN_TARGET_Y = 525;
...
function assertCountdownFontReady() {
  if (
    ...
    throw new Error("正式倒數字型尚未載入，已停止 C－10 Template render。");
  }
}
...
function drawCountdown(context, text) {
  ...
  if (!metricValues.every(Number.isFinite)) {
    throw new Error("瀏覽器無法取得 C－10 倒數文字的正式 glyph metrics。");
  }
  ...
  if (inkRight <= inkLeft || inkBottom <= inkTop) {
    throw new Error("C－10 倒數文字沒有可用的正式 glyph ink bounds。");
  }
...
export async function waitForPopUpFonts() {
  await waitForAPopUpFonts();
  if (!document.fonts) {
    throw new Error("瀏覽器不支援正式字型載入檢查，已停止 C－10 Template render。");
  }
...
export function renderPopUp(
  ...
  if (!isValidCCountdown(cCountdownText)) {
    throw new TypeError("C－10 倒數天數只允許完整字串 0天～9天。");
  }

  assertCountdownFontReady();
  renderAPopUp(canvas, backgroundImage, {
    headline,
    subheadline,
    protectionText
  });
  ...
  if (canvas.width !== POP_UP_WIDTH || canvas.height !== POP_UP_HEIGHT) {
    throw new Error("C－10 正式 Canvas 必須為 580 × 720px。");
  }
  const context = canvas.getContext("2d");
  if (!context) throw new Error("無法建立 C－10 Canvas 2D context。");

  drawCountdown(context, cCountdownText);
}

export { POP_UP_HEIGHT, POP_UP_WIDTH };
